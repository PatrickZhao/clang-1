...
low-level interface to the indexing library. It approximates the Clang API
while simultaneously being "Pythonic."

Notable differences from the C API are:
...
This module provides automatic memory management of objects generated by the
Clang API. If an object goes out of scope, the appropriate Clang API will be
called to dispose of it. References to parent objects are stored transparently
in created objects, so the caller doesn't need to worry about object lifetime
management or holding on to references to prevent a premature garbage
collection. If you find this contract is not being upheld, please file a bug.
...
TranslationUnit
  ...
  source code can be parsed on the fly using Python.
...
* The high-level Indexing feature is not implemented.
